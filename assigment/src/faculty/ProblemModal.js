// Updated ProblemModal.js
import React, { useState } from 'react';
import axios from '../api/axios'; // Axios instance
import { Plus, X } from 'lucide-react';
import { useAppData } from './shared'; // Assuming this provides admin-added questions and batches

export function ProblemModal({ problem, onClose, refetchAssignments }) {
  const { problems: adminQuestions, batches = ['Batch A', 'Batch B', 'Batch C'] } = useAppData(); // Pull admin-added questions and batches[14]
  ...
  // Load from admin question bank
  const handleSelectAdminQuestion = (id) => {
    const selected = adminQuestions.find(q => q.id === id);
    if (selected) {
      setTitle(selected.title);
      setDescription(selected.description);
      setDifficulty(selected.difficulty);
      setTestCases(selected.testCases.map(tc => ({ ...tc, hidden: tc.hidden || false })));
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      testCases: testCases.map(tc => ({ ...tc })) // Hidden flag preserved for evaluation[19]
    };
    try {
      if (problem?._id) {
        await axios.put(`/assignments/update/${problem._id}`, payload, { headers: { Authorization: `Bearer ${token}` } });
        alert('✅ Assignment updated!');
      } else {
        await axios.post('/assignments/create', payload, { headers: { Authorization: `Bearer ${token}` } });
        ...
        <form className="modal-form" onSubmit={handleSubmit}>
          <div className="modal-form-content">
            ...
            <div className="form-group">
              ...
              <select onChange={(e) => handleSelectAdminQuestion(e.target.value)}>
            ...
            <div className="test-case-list">
              {testCases.map((tc, index) => (
                <div key={index} className="test-case-item">
                  <div className="test-case-header">Test Case {index + 1} <button type="button" className="remove-testcase-btn" onClick={() => handleRemoveTestCase(index)}><X size={16} /></button></div>
                  <div className="test-case-fields">
                    ...
                    <div>
                      <label>Hidden (not visible to students)</label>
            ...
            <button type="button" className="add-testcase-btn" onClick={handleAddTestCase}><Plus size={16} /> Add Test Case</button>
          </div>
